...
return view('frontend.pages.job.job')->withAlljobs($alljobs)
->withJob_function($job_function);
}
 }


public function show_alert_jobs()
    {          
               $alljobs=Job::orderBy('id','desc')->get();
               $alert_jobs = DB::table('alert_job_categories')
...
if (Auth::check()) {
    ...
    $group=DB::table('groups')->where('users_id', '=', Auth::user()->id)->get();
      ...
      if (!empty($group)) {         
            ...
            return view('frontend.pages.job.show_all_alert_jobs')
            ->withGroup($group)
            ->withAlljobs($alljobs)
            ->withAlert_jobs($alert_jobs)
            ->withCheck_save_job($check_save_job);
          }
          else{
        return view('frontend.pages.job.show_all_alert_jobs')
                  ->withAlljobs($alljobs)
...
$this->validate($request, [
            ...
            'activities'=>'required|min:3',
            'binefits' => 'required|min:3',
...
if(!empty($request->jobs_type)){
   $builder->WhereIn('jobs_type',$request->jobs_type);
}
if(!empty($salary)){
  $full = $salary;
  foreach($full as $results){
if ($results ==='range1') {
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [0, 10000]);
        });
      
}
 
elseif($results === "range2"){
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [10000,20000]);
        });
      
}
elseif($results === "range3"){
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [20000,30000]);
        });
      
}
elseif($results === "range4"){
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [30000,40000]);
        });
      
}
elseif($results === "range5"){
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [40000,50000]);
        });
      
}
elseif($results === "range6"){
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [50000,70000]);
        });
      
}
else{
     $builder->Where(function ($query) {
            $query->whereBetween('salary1', [70000,500000]);
        });
