 ...
 <div class="member">
     ...
     <button class="members"><a>Member</a></button>
       ...
       <div class="addmemberdesc">
         ...
         <input type="hidden" name="group_id" value="{{$current_group->id}}">
    {{ Form::text('tags', '', ['id' =>  'tags','required'=>'required', 'placeholder' =>  'Search users'])}}
...
{{ Form::close() }}
       ...
       <div class="membersdes ">
         ...
         <span class="mem-num text-center"><a style="text-decoration: none;" href="{{url('/group-members',$current_group->id)}}">Total <span class="label label-success">{{count($all_group_member)}} member</span></a></span>
         <div class="row">
         @foreach($six_group_member as $six_group_members)
         <div class="col-md-2">
         <div class="img">
           <img src="{!! asset('images/user.jpg') !!}" class="img-responsive">
          @foreach($all_user as $all_users)
           
           @if($all_users->id == $six_group_members->users_id)
           <p>{{$all_users->name}}</p>
           @endif
           @endforeach
           </div>
         ...
         <p class="more"><a href="{{url('/group-members',$current_group->id)}}">see more</a></p>
       </div>
     </div> 
